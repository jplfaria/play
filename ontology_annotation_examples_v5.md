...
This document demonstrates how to annotate microbial phenotype data using an association-based model, with parallel examples showing three approaches:
1. **Version A**: Using only currently available ontologies (CHEBI, GO, PATO, ECO, NCBITaxon, RO, UO, ENVO, OBI)
2. **Version B**: Adding both OMP (Ontology of Microbial Phenotypes) and MCO (Microbial Conditions Ontology)
3. **Version C**: Using OMP but not MCO (hybrid approach)
...
#### Version B: With OMP and MCO (Specialized Ontologies)
...
#### Version C: With OMP but without MCO (Hybrid Approach)
...
#### Version C: With OMP but without MCO
...
association:
  ...
  qualifiers:
    ...
    condition_qualifier:
      id: "MCO:0000032"  # LB medium, Miller
      extensions:
...
association:
  ...
  qualifiers:
    ...
    substrate_qualifier: "CHEBI:17814; modelseed.compound:cpd01030"  # salicin
    growth_medium:
      type: "ENVO:01001059"  # microbial culture medium
...
1. **Version A (Without OMP/MCO)**: Requires 3-5 ontology terms to construct each phenotype
2. **Version B (With OMP and MCO)**: Single phenotype term + state qualifier, plus standardized condition terms
3. **Version C (With OMP only)**: Single phenotype term + state qualifier, but conditions in free text/general ontologies
4. **Evidence remains consistent** across all approaches using ECO
5. **Complexity reduction**: 
   - Version B: ~60% fewer terms needed when pre-composed terms exist
   - Version C: ~40% fewer terms while maintaining flexibility for condition descriptions
